...
export const getApartmentColumns = (
  isOwnerDetailsPage = false
): ColumnDef<ApartmentModel>[] => {
  // Define the main columns from "Title" to "Created Date"
  const baseColumns: ColumnDef<ApartmentModel>[] = [
    {
      accessorKey: "title",
      header: ({ column }) => (
  ...
  // Conditionally add "Cover" column at the beginning if not on the owner details page
  if (!isOwnerDetailsPage) {
    baseColumns.unshift({
    ...
    // Add "Actions" column at the end if not on the owner details page
    baseColumns.push({
      accessorKey: "actions",
      header: "Actions",
      cell: ({ row }) => {
